...
const OfferCard = ({ details,customCSS }) => {
  return (
    <div className={` ${customCSS}`} style={{ backgroundImage: `url(${details.imageURL})` }}>
      {details.subTitle && <span className="text-12px font-bold uppercase text-red">{details.subTitle}</span>}
      {details.title && <h4 className="text-28px max-lg:text-24px font-bold capitalize text-black">{details.title}</h4>}
      {details.descrpt && <p className="text-14px max-lg:text-13px max-md:text-12px font-semibold capitalize text-black">{details.descrpt}</p>}
